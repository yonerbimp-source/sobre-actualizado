...
let pendingVoiceNotes = { add: [], remove: [], edit: [] };
let activeRecorder = null;
let activeRecorderStream = null;
let activeRecorderContext = null;
let activeRecorderTimer = null;
let activeRecorderSeconds = 0;
...
const t = {
  es: {
    ...
    langButton: 'English', income: 'entrada', expense: 'salida', includedInTotal: 'Cuenta en el total', excludedFromTotal: 'No cuenta en el total', totalNote: 'Solo suma los sobres marcados para el total', toggleTotalOk: 'Preferencia del total actualizada.', editTransaction: 'Editar', editTransactionTitle: 'Editar transacción', updateTransaction: 'Guardar cambios', transactionUpdated: 'Transacción actualizada correctamente.', deleteTransaction: 'Eliminar', deleteTransactionConfirm: '¿Seguro que deseas eliminar esta transacción?', transactionDeleted: 'Transacción eliminada correctamente.', voiceNote: 'Nota de voz', recordVoice: 'Grabar nota de voz', stopRecording: 'Detener grabación', recording: 'Grabando: {seconds}s de 60s', voiceReady: 'Nota de voz lista', deleteVoice: 'Eliminar nota', deleteVoiceConfirm: '¿Seguro que deseas eliminar esta nota de voz?', microphoneError: 'No se pudo usar el micrófono. Revisa el permiso de la aplicación.', voiceDeleted: 'Nota de voz eliminada.', voiceLimit: 'La grabación llegó al límite de 1 minuto.'
  },
  en: {
    ...
    langButton: 'Español', income: 'income', expense: 'expense', includedInTotal: 'Counts in total', excludedFromTotal: 'Does not count in total', totalNote: 'Only envelopes marked for the total are added', toggleTotalOk: 'Total preference updated.', editTransaction: 'Edit', editTransactionTitle: 'Edit transaction', updateTransaction: 'Save changes', transactionUpdated: 'Transaction updated successfully.', deleteTransaction: 'Delete', deleteTransactionConfirm: 'Are you sure you want to delete this transaction?', transactionDeleted: 'Transaction deleted successfully.', voiceNote: 'Voice note', recordVoice: 'Record voice note', stopRecording: 'Stop recording', recording: 'Recording: {seconds}s of 60s', voiceReady: 'Voice note ready', deleteVoice: 'Delete note', deleteVoiceConfirm: 'Are you sure you want to delete this voice note?', microphoneError: 'The microphone could not be used. Check the app permission.', voiceDeleted: 'Voice note deleted.', voiceLimit: 'The recording reached the 1-minute limit.'
  }
};
...
function addEnvelope() {
  ...
  data.envelopes.push({ id: createId(), name, color, balance: 0, includeInTotal: true, history: [] });
...
function addMoney() {
  ...
  envelope.history.unshift({ id: createId(), type: 'entrada', amount, description, date: nowText(), voiceNotes: pendingVoiceNotes.add });
  document.getElementById('addAmount').value = '';
  document.getElementById('addDescription').value = '';
  pendingVoiceNotes.add = [];
  renderPendingVoiceNotes('add');
  saveData();
  render();
...
function removeMoney() {
  ...
  if (!description) return showMessage(tr('descriptionRequired'), 'error');
  envelope.balance -= amount;
  envelope.history.unshift({ id: createId(), type: 'salida', amount, description, date: nowText(), voiceNotes: pendingVoiceNotes.remove });
  document.getElementById('removeAmount').value = '';
  document.getElementById('removeDescription').value = '';
  pendingVoiceNotes.remove = [];
  renderPendingVoiceNotes('remove');
  saveData();
  render();
...
function renderHistoryList(history) {
  ...
  list.innerHTML = history.map(m => `
    <div class="move">
      ...
      ${renderVoiceNotes(m.voiceNotes || [], m.id, false)}
      <div class="move-actions">
        <button class="edit-move-btn" onclick="editTransaction('${m.id}')">${tr('editTransaction')}</button>
        <button class="delete-move-btn" onclick="deleteTransaction('${m.id}')">${tr('deleteTransaction')}</button>
      </div>
    </div>`).join('');
}
...
function deleteTransaction(transactionId) {
  ...
  if (!envelope) return;
  const transactionIndex = envelope.history.findIndex(m => m.id === transactionId);
  if (transactionIndex === -1) return;
  if (!confirm(tr('deleteTransactionConfirm'))) return;

  const transaction = envelope.history[transactionIndex];
  envelope.balance += transaction.type === 'entrada' ? -transaction.amount : transaction.amount;
  envelope.history.splice(transactionIndex, 1);

  saveData();
  render();
  openHistory(envelope.id);
  showMessage(tr('transactionDeleted'), 'ok');
}
...
function editTransaction(transactionId) {
  ...
  pendingVoiceNotes.edit = (transaction.voiceNotes || []).map(note => ({ ...note }));
  renderPendingVoiceNotes('edit');
...
function updateTransaction() {
  ...
  transaction.voiceNotes = pendingVoiceNotes.edit;
...
function renderVoiceNotes(notes, transactionId, readOnly = false) {
  if (!notes.length) return '';
  return `<div class="voice-notes">${notes.map((note, index) => `
    <div class="voice-note-item">
      <span>${tr('voiceNote')} ${index + 1}</span>
      <audio controls preload="metadata" src="${note.dataUrl}"></audio>
      ${readOnly ? '' : `<button class="delete-voice-btn" onclick="deleteSavedVoiceNote('${transactionId}', '${note.id}')">${tr('deleteVoice')}</button>`}
    </div>`).join('')}</div>`;
}

function renderPendingVoiceNotes(context) {
  const box = document.getElementById(`${context}VoiceNotes`);
  if (!box) return;
  const notes = pendingVoiceNotes[context] || [];
  box.innerHTML = notes.map((note, index) => `
    <div class="voice-note-item">
      <span>${tr('voiceNote')} ${index + 1}</span>
      <audio controls preload="metadata" src="${note.dataUrl}"></audio>
      <button type="button" class="delete-voice-btn" onclick="deletePendingVoiceNote('${context}', '${note.id}')">${tr('deleteVoice')}</button>
    </div>`).join('');
}

function deletePendingVoiceNote(context, noteId) {
  if (!confirm(tr('deleteVoiceConfirm'))) return;
  pendingVoiceNotes[context] = (pendingVoiceNotes[context] || []).filter(note => note.id !== noteId);
  renderPendingVoiceNotes(context);
}

function deleteSavedVoiceNote(transactionId, noteId) {
  if (!confirm(tr('deleteVoiceConfirm'))) return;
  const envelope = data.envelopes.find(e => e.history.some(m => m.id === transactionId));
  if (!envelope) return;
  const transaction = envelope.history.find(m => m.id === transactionId);
  if (!transaction) return;
  transaction.voiceNotes = (transaction.voiceNotes || []).filter(note => note.id !== noteId);
  saveData();
  openHistory(envelope.id);
  showMessage(tr('voiceDeleted'), 'ok');
}

function updateRecorderUi(context) {
  const button = document.getElementById(`${context}RecordBtn`);
  const status = document.getElementById(`${context}RecordStatus`);
  if (!button || !status) return;
  const active = activeRecorder && activeRecorder.state === 'recording' && activeRecorderContext === context;
  button.textContent = active ? tr('stopRecording') : tr('recordVoice');
  button.classList.toggle('recording', active);
  status.textContent = active ? tr('recording', { seconds: activeRecorderSeconds }) : '';
}

async function toggleVoiceRecording(context) {
  if (activeRecorder && activeRecorder.state === 'recording') {
    if (activeRecorderContext === context) activeRecorder.stop();
    return;
  }
  if (!navigator.mediaDevices?.getUserMedia || !window.MediaRecorder) {
    return showMessage(tr('microphoneError'), 'error');
  }
  try {
    activeRecorderStream = await navigator.mediaDevices.getUserMedia({ audio: true });
    const preferredTypes = ['audio/webm;codecs=opus', 'audio/webm', 'audio/mp4'];
    const mimeType = preferredTypes.find(type => MediaRecorder.isTypeSupported(type)) || '';
    activeRecorder = mimeType ? new MediaRecorder(activeRecorderStream, { mimeType, audioBitsPerSecond: 16000 }) : new MediaRecorder(activeRecorderStream, { audioBitsPerSecond: 16000 });
    activeRecorderContext = context;
    activeRecorderSeconds = 0;
    const chunks = [];
    activeRecorder.ondataavailable = event => { if (event.data.size) chunks.push(event.data); };
    activeRecorder.onstop = () => {
      clearInterval(activeRecorderTimer);
      const recordedContext = activeRecorderContext;
      const blob = new Blob(chunks, { type: activeRecorder.mimeType || 'audio/webm' });
      const reader = new FileReader();
      reader.onloadend = () => {
        pendingVoiceNotes[recordedContext].push({ id: createId(), dataUrl: reader.result, createdAt: nowText() });
        renderPendingVoiceNotes(recordedContext);
        showMessage(tr('voiceReady'), 'ok');
      };
      reader.readAsDataURL(blob);
      activeRecorderStream?.getTracks().forEach(track => track.stop());
      activeRecorder = null;
      activeRecorderStream = null;
      activeRecorderContext = null;
      updateRecorderUi(recordedContext);
    };
    activeRecorder.start(250);
    updateRecorderUi(context);
    activeRecorderTimer = setInterval(() => {
      activeRecorderSeconds += 1;
      updateRecorderUi(context);
      if (activeRecorderSeconds >= MAX_VOICE_SECONDS && activeRecorder?.state === 'recording') {
        activeRecorder.stop();
        showMessage(tr('voiceLimit'), 'ok');
      }
    }, 1000);
  } catch (error) {
    activeRecorderStream?.getTracks().forEach(track => track.stop());
    ...
    activeRecorderStream = null;
    activeRecorderContext = null;
    showMessage(tr('microphoneError'), 'error');
    updateRecorderUi(context);
  }
}
    ...
    <div class="deleted-item">
      ...
      ${e.history.length ? e.history.map(m => `
        <div class="move">
          ...
          <div>${m.date}</div>
          <p>${escapeHtml(m.description)}</p>
          ${renderVoiceNotes(m.voiceNotes || [], m.id, true)}
